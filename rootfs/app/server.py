...
def get_p():
	if 'img' in request.args:
		try:
			...
			threshold = float(request.args.get('threshold', DEFAULT_THRESHOLD)) # Get the threshold from the request params, default to 0.08
...
def failure_detect():
	try:
		...
		threshold = float(data.get("threshold", DEFAULT_THRESHOLD)) # Get the threshold from the request data, default to 0.08
...
# /check/ endpoint
# example request: http://localhost:3333/check/?entity_id=camera.example_camera
# example response: {"detections": [["failure", 0.627044320106506, [[314.500637054443, 594.114170074463, 175.366888046265, 293.919010162354]], ["failure", 0.351168990135193, [177.455763816834, 361.810432434082, 233.478097915649, 292.509155273438]]]}
...
def check_entity():
	if 'entity_id' in request.args:
		try:
			...
			return jsonify({"error": f"Failed to process entity image - {str(e)}"}), 500
	else:
		app.logger.warn("Invalid request params: {}".format(request.args))
